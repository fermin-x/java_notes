...
import com.fermin.redis.lock.handler.KlockInvocationException;
...
public class LockAspectHandler {
    ...
    public Object around(ProceedingJoinPoint joinPoint, GlobalLock klock) throws Throwable {
    ...
    public void afterReturning(JoinPoint joinPoint, GlobalLock klock) throws Throwable {
    ...
    public void afterThrowing(JoinPoint joinPoint, GlobalLock klock, Throwable ex) throws Throwable {
        String curentLock = this.getCurrentLockId(joinPoint, klock);
        releaseLock(klock, joinPoint, curentLock);
        cleanUpThreadLocal(curentLock);
        throw ex;
     ...
     * 释放锁
     */
    private void releaseLock(GlobalLock klock, JoinPoint joinPoint, String curentLock) throws Throwable {
     ...
     * @return
     */
    private String getCurrentLockId(JoinPoint joinPoint, GlobalLock klock) {
        LockInfo lockInfo = lockInfoProvider.get(joinPoint, klock);
     ...
     * 处理释放锁时已超时
     */
    private void handleReleaseTimeout(GlobalLock klock, LockInfo lockInfo, JoinPoint joinPoint) throws Throwable {
